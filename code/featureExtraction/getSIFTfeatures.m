function f,d=  getSIFTfeatures(aMask, img, salMap, weighted)
...
% weighted is a boolean to see if this sift source image should be modified by salMap weight
 ...
 %    descriptor is a 128-dimensional vector of class UINT8.
	...
	% now extract SIFT from this area

	if weighted
		[f,d] = vl_sift(single(double(gimg).*double(segMask).*double(salMap))) ;
	else
		[f,d] = vl_sift(single(double(gimg).*double(segMask))) ;
	end
